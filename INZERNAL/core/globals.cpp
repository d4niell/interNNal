...
//logging options
namespace logging {
		bool	enabled		= false; //master switch for all logging
...
namespace opt {
	...
	std::string	gt_version	= "4.20";	//gt version gets spoofed to this
	std::string	gt_supported= "3.70";	//the gt version for which INZERNAL was built for. Probably wont work for other versions.

	bool		spoof_login	= true;		//whether or not to spoof mac, hash2, wk, rid, etc on login. GT version spoofed even when off.
	bool		spoof_name	= false;		//Also set the requestedName field to random. Might be unwanted for non grow-id accounts.
	bool		login		= false;
	...
	std::string	flag		= "ge";		//set if flag_mode is set to fmCustom
	...
	namespace cheat {	
		...
		bool	tp_click			= false;		//when on, holding CTRL allows you to tp to mouse location.
		bool	see_ghosts			= false;		//allows you to see ghosts so you can avoid constant slime
		...
		bool	antipunch			= false;		//basically (personal) punch jammer even when there is none
...
namespace global {
		...
		bool		draw	= true;	//	menu draw status. set to true for menu to show up when you inject
		std::string	version	= "V0.0	private	[alpha]";//	INZERNAL version
